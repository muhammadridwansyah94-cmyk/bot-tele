...
async def send_sms_async(msg_text, reply_markup, phone, otp, api_name, sent_sms_ids, sms_id):
    now = datetime.utcnow().timestamp()
    force_send = sms_id not in sent_sms_ids or (now - sent_sms_ids.get(sms_id, 0)) > CLEANUP_HOURS * 3600

    if not force_send:
        print(f"[DEBUG] SMS {sms_id} recently sent, skipping")
        return
...
async def fetch_api(session, api, sent_sms_ids):
    while True:
        try:
            ...
            async with session.get(api["url"], headers=headers, timeout=15) as response:
                ...
                if isinstance(raw_data, list):
                    ...
                    # dict tunggal yang mirip entry SMS
                    if all(k in raw_data for k in ("cli", "num", "message")):
                        data = [raw_data]
                    else:
                        # cari value list di dict
                        data = next((v for v in raw_data.values() if isinstance(v, list)), [])
                        if not data:
                            print(f"[WARNING] {api['name']} dict response tapi tidak ada list atau entry, wrapping empty list")
                else:
                    data = []
        ...
        print(f"[DEBUG] {api['name']} processing {len(entries)} entries | IDs: {[e.get('id') for e in entries]}")
        ...
        for entry in entries:
            sms_id = str(entry.get("id", datetime.utcnow().timestamp()))  # fallback id unik kalau dict tunggal tanpa id
