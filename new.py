...
# Telegram info dari ENV (lebih aman di Railway)
TELEGRAM_BOT_TOKEN = os.getenv("TELEGRAM_SEND_TOKEN")
TELEGRAM_CHAT_ID = int(os.getenv("TELEGRAM_SEND_CHAT_ID"))
...
# Country map (disingkat saja untuk contoh)
country_map = {
    "1": ("United States", "🇺🇸"),
    "62": ("Indonesia", "🇮🇩"),
    "91": ("India", "🇮🇳"),
    # tambahkan negara lain sesuai scriptmu...
}
...
while True:
    ...
    for entry in new_entries[::-1]:
        ...
        otp = "N/A"
        otp_match = re.search(
            r'(?:code|كود|رمز|كود التفعيل|رمز التحقق|código|код|验证码|code de vérification|codice|verification code|Your .* code)[\s\W:-]*(\d{3,8})',
