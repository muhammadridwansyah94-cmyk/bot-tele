...
country_map = {
    ...
    "976": ("MN", "🇲🇳"), "977": ("NP", "🇳🇵"), "979": ("🌎", "🌎"), "98": ("IR", "🇮🇷"),
    "991": ("🌎", "🌎"), "992": ("TJ", "🇹🇯"), "993": ("TM", "🇹🇲"), "994": ("AZ", "🇦🇿"),
...
lang_map = {
    ...
    "xh": "Xhosa", "yi": "Yiddish", "zh-cn": "Chinese (Simplified)", "zh-tw": "Chinese (Traditional)",
    "zu": "Zulu"
}
...
def extract_otp(message):
    if not message:
        return "N/A"
    keywords = ["code", "kode", "otp", "verification number"]
    for kw in keywords:
        pattern = re.compile(rf"(?i){kw}[:\s]*")
        match = pattern.search(message)
        if match:
            following_text = message[match.end():]
            digits = re.findall(r'\d{2,8}', following_text)
            if digits:
                return ''.join(digits)
        ...
        for api in API_LIST:
            ...
            while retries > 0:
                try:
                    ...
                    if not data:
                        break
                    if api["name"] == "TIMESMS" and "data" in data:
                        data = data["data"]
                    for item in data:
                        if api["name"] == "TIMESMS":
                            app_name = item.get("cli", "TIMESMS")
                            phone = item.get("num", "")
