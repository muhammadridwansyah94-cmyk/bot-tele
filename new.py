...
# ==== COUNTRY MAP & LANG MAP ==== (sama seperti sebelumnya)
country_map = { "1": ("US", "🇺🇸"),
    "1242": ("BS", "🇧🇸"),
    "1246": ("BB", "🇧🇧"),
    "1264": ("AI", "🇦🇮"),
...
# ===== CHECK SMS DARI MULTI API DENGAN RETRY TIMESMS =====
async def check_sms():
    ...
    async with aiohttp.ClientSession() as session:
        for api in API_LIST:
            try:
                data = None
                retries = 0
                max_retries = 2 if api['name'] == "TIMESMS" else 0
                    ...
                    try:
                        async with session.get(api["url"], params={"token": api["token"], "records": ""}, timeout=15) as resp:
                            try:
                                ...
                                print(f"[{api['name']}] Invalid JSON (attempt {retries+1}): {text[:200]}")
                                retries += 1
                                if retries > max_retries:
                                    data = None
                                    print(f"[{api['name']}] Skipping after {max_retries+1} failed attempts")
                                    break
...
async def stats_command():
    await bot.send_message(
        chat_id=CHAT_ID,
        text=f"📊 Total SMS tracked: {len(sent_messages)}",
        parse_mode=ParseMode.MARKDOWN_V2
    )
