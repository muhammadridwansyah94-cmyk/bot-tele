...
country_codes = { '1': ('USA/Canada (NANP)', '🇺🇸'),
    ...
    '996': ('Kyrgyzstan', '🇰🇬'),
    '998': ('Uzbekistan', '🇺🇿'),
# ... (paste semua kode negara kamu disini tanpa ubah)
}
...
# ================= FETCH API ==================
async def fetch_api(session, api, sent_sms_ids, last_sms_ids):
    while True:
        try:
            ...
            async with session.get(api["url"], params=params, timeout=40) as resp:
                ...
                # Tandai ID terakhir tapi **jangan kirim**
                if api["name"] not in last_sms_ids:
                    last_sms_ids[api["name"]] = f"{entries[-1]['dt']}_{entries[-1]['num']}_{entries[-1]['cli']}"
                    await asyncio.sleep(5)
                    continue

                for entry in entries:
                    sms_id = f"{entry['dt']}_{entry['num']}_{entry['cli']}"
                    if sms_id in sent_sms_ids or sms_id <= last_sms_ids[api["name"]]:
                        continue
...
async def main_loop():
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [fetch_api(session, api, sent_sms_ids, last_sms_ids) for api in APIS]
...
if __name__ == "__main__":
    ...
    print("✅ OTP Auto Forwarder Running (New SMS Only)")
