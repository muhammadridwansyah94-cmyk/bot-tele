...
country_codes = { 
    ...
    '996': ('Kyrgyzstan', '🇰🇬'),
    '998': ('Uzbekistan', '🇺🇿'),
    # ... paste negara lengkap seperti script asli tanpa diubah ...
}
...
async def send_sms_async(msg_text, reply_markup, phone, api_name):
    ...
    while attempt < MAX_RETRY:
        try:
            async with lock:  # pastikan tidak duplikat async
    ...
    while True:
        try:
            ...
            async with session.get(api["url"], params=params, timeout=40) as resp:
                ...
                # FIRST START → 1 SMS TERAKHIR
                if not first_fetch_done[api["name"]]:
                    latest = entries[-1]
                    sms_id = f"{latest['num']}_{latest['cli']}_{latest['message']}"  # Normalisasi ID
                ...
                for entry in entries:
                    sms_id = f"{entry['num']}_{entry['cli']}_{entry['message']}"  # Normalisasi ID
