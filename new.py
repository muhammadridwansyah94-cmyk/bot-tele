...
async def send_sms_async(msg_text, reply_markup, phone, otp, api_name, sent_sms_ids, sms_id):
    ...
    while attempt < MAX_RETRY:
        try:
            ...
            log_message(f"Telegram RetryAfter: wait {e.retry_after} seconds")
...
def extract_entries(raw_data):
    """
    Extract entries from any JSON format to dicts with keys:
    'cli', 'num', 'message'.
    Supports list of dicts, list of lists, nested dicts, or mixed formats.
    """
    entries = []
    ...
    if isinstance(raw_data, list):
        for e in raw_data:
            if isinstance(e, dict):
                ...
                cli = e[0] if len(e) > 0 else "Unknown"
                num = e[1] if len(e) > 1 else ""
                message = e[2] if len(e) > 2 else ""
                if num and message:
                    entries.append({"cli": cli, "num": num, "message": message})
        ...
        try:
            ...
            async with session.get(api["url"], headers=headers, timeout=15) as response:
                if response.status != 200:
                    log_message(f"[{api['name']}] API returned status {response.status}")
