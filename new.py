...
SMS_DELAY = 0.5
MAX_RETRY = 5
PERSIST_FILE = "last_sms.json"

bot = Bot(token=TELEGRAM_BOT_TOKEN)
lock = asyncio.Lock()  # untuk mencegah double async
...
country_codes = { '1': ('USA/Canada (NANP)', '🇺🇸'),
    ...
    '995': ('Georgia', '🇬🇪'),
    '996': ('Kyrgyzstan', '🇰🇬'),
    '998': ('Uzbekistan', '🇺🇿'), }  # tetap seperti script kamu (potong di sini untuk singkat)
...
async def send_sms_async(msg_text, reply_markup, phone, api_name):
    ...
    while attempt < MAX_RETRY:
        try:
            async with lock:  # pastikan tidak double
...
# ---------------- FETCH API ----------------
async def fetch_api(session, api, last_sms):
    while True:
        try:
            ...
            async with session.get(api["url"], params=params, timeout=40) as resp:
                ...
                # Kirim hanya SMS terakhir saat pertama kali
                last_id = last_sms.get(api["name"])
                latest = entries[-1]
                ...
                if last_id != sms_id:
                    ...
                    last_sms[api["name"]] = sms_id
                    save_last_sms(last_sms)

                # Kirim SMS baru yang muncul setelah ini
                for entry in entries:
                    sms_id = f"{entry['dt']}_{entry['num']}_{entry['cli']}"
                    if last_sms.get(api["name"]) == sms_id:
                        continue
...
async def main_loop():
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [fetch_api(session, api, last_sms) for api in APIS]
...
if __name__ == "__main__":
    ...
    flask_thread.start()
    print("✅ OTP Auto Forwarder Running (Last SMS Only + No Duplicate)")
