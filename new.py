    ...
    while True:
        try:
            ...
            async with session.get(api["url"], headers=headers, timeout=15) as response:
                ...
                print(f"[DEBUG] {api['name']} raw data type: {type(raw_data)}")

                # --- FLEXIBLE WRAPPER UNTUK SEMUA FORMAT ---
                data = []

                if isinstance(raw_data, list):
                    # cek list of lists (Botsms)
                    if all(isinstance(e, list) and len(e) >= 3 for e in raw_data):
                        for e in raw_data:
                            entry = {
                                "cli": e[0],
                                "num": e[1],
                    ...
                    # dict tunggal dengan field cli/num/message
                    if all(k in raw_data for k in ("cli","num","message")):
                        data = [raw_data]
                    else:
                        # dict nested -> cari list di value
                        for v in raw_data.values():
                            if isinstance(v, list):
                                data = v
                                break
                        if not data:
                            print(f"[WARNING] {api['name']} dict response tapi tidak ada list/entry")
                else:
                    print(f"[WARNING] {api['name']} response bukan list atau dict, skipping")
        ...
        entries = sorted(data, key=lambda x: x.get("id", datetime.utcnow().timestamp()), reverse=True)[:5]
        ...
        for entry in entries:
            sms_id = str(entry.get("id", datetime.utcnow().timestamp()))
