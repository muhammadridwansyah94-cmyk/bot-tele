...
# ================== KONFIG ==================
BOT_TOKEN = "8629130600:AAEI5OaS5UFOnKE4yL3j3FXPvoMqJxcHiss"
...
country_map = {
    ...
    "1649": ("TC", "🇹🇨"), "1658": ("JM", "🇯🇲"), "1664": ("MS", "🇲🇸"), "1670": ("MP", "🇲🇵"), "1671": ("GU", "🇬🇺"),
    "1684": ("AS", "🇦🇸"), "1721": ("SX", "🇸🇽"), "1758": ("LC", "🇱🇨"), "1767": ("DM", "🇩🇲"), "1784": ("VC", "🇻🇨"),
    "1787": ("PR", "🇵🇷"), "1809": ("DO", "🇩🇴"), "1829": ("DO", "🇩🇴"), "1849": ("DO", "🇩🇴"), "1868": ("TT", "🇹🇹"),
    "1869": ("KN", "🇰🇳"), "1876": ("JM", "🇯🇲"), "1907": ("US", "🇺🇸"), "1939": ("PR", "🇵🇷"), "20": ("EG", "🇪🇬"),
    ...
    "65": ("SG", "🇸🇬"), "66": ("TH", "🇹🇭"), "670": ("TL", "🇹🇱"), "672": ("AU", "🇦🇺"), "673": ("BN", "🇧🇳"),
    "674": ("NR", "🇳🇷"), "675": ("PG", "🇵🇬"), "676": ("TO", "🇹🇴"), "677": ("SB", "🇸🇧"), "678": ("VU", "🇻🇺"),
    "679": ("FJ", "🇫🇯"), "680": ("PW", "🇵🇼"), "681": ("WF", "🇼🇫"), "682": ("CK", "🇨🇰"), "683": ("NU", "🇳🇺"),
    "685": ("WS", "🇼🇸"), "686": ("KI", "🇰🇮"), "687": ("NC", "🇳🇨"), "688": ("TV", "🇹🇻"), "689": ("PF", "🇵🇫"),
    "690": ("TK", "🇹🇰"), "691": ("FM", "🇫🇲"), "692": ("MH", "🇲🇭"), "693": ("UM", "🇺🇲"), "694": ("UM", "🇺🇲"),
    "695": ("🌎", "🌎"), "696": ("PN", "🇵🇳"), "697": ("NF", "🇳🇫"), "698": ("CX", "🇨🇽"), "699": ("CC", "🇨🇨"),
    "7": ("RU", "🇷🇺"), "81": ("JP", "🇯🇵"), "82": ("KR", "🇰🇷"), "84": ("VN", "🇻🇳"), "850": ("KP", "🇰🇵"),
    "852": ("HK", "🇭🇰"), "853": ("MO", "🇲🇴"), "855": ("KH", "🇰🇭"), "856": ("LA", "🇱🇦"), "86": ("CN", "🇨🇳"),
    "870": ("🌎", "🌎"), "878": ("🌎", "🌎"), "880": ("BD", "🇧🇩"), "881": ("🌎", "🌎"), "882": ("🌎", "🌎"),
    "883": ("🌎", "🌎"), "886": ("TW", "🇹🇼"), "888": ("🌎", "🌎"), "89": ("🌎", "🌎"), "90": ("TR", "🇹🇷"),
...
lang_map = {
    ...
    "bn": "Bengali","bs": "Bosnian","ca": "Catalan","cs": "Czech","cy": "Welsh","da": "Danish","de": "German",
    "el": "Greek","en": "English","eo": "Esperanto","es": "Spanish","et": "Estonian","eu": "Basque","fa": "Persian",
    "fi": "Finnish","fr": "French","ga": "Irish","gl": "Galician","gu": "Gujarati","he": "Hebrew","hi": "Hindi",
    "hr": "Croatian","ht": "Haitian Creole","hu": "Hungarian","hy": "Armenian","id": "Indonesian","is": "Icelandic",
    "it": "Italian","ja": "Japanese","jv": "Javanese","ka": "Georgian","kk": "Kazakh","km": "Khmer","kn": "Kannada",
    "ko": "Korean","ku": "Kurdish","ky": "Kyrgyz","la": "Latin","lb": "Luxembourgish","lo": "Lao","lt": "Lithuanian",
    "lv": "Latvian","mg": "Malagasy","mi": "Maori","mk": "Macedonian","ml": "Malayalam","mn": "Mongolian","mr": "Marathi",
    "ms": "Malay","mt": "Maltese","my": "Burmese","ne": "Nepali","nl": "Dutch","no": "Norwegian","pa": "Punjabi",
    "pl": "Polish","ps": "Pashto","pt": "Portuguese","ro": "Romanian","ru": "Russian","sd": "Sindhi","si": "Sinhala",
    "sk": "Slovak","sl": "Slovenian","so": "Somali","sq": "Albanian","sr": "Serbian","su": "Sundanese","sv": "Swedish",
    "sw": "Swahili","ta": "Tamil","te": "Telugu","tg": "Tajik","th": "Thai","tl": "Filipino","tr": "Turkish","uk": "Ukrainian",
    "ur": "Urdu","uz": "Uzbek","vi": "Vietnamese","xh": "Xhosa","yi": "Yiddish","zh-cn": "Chinese (Simplified)","zh-tw": "Chinese (Traditional)","zu": "Zulu"
}
...
def extract_otp(message):
    if not message:
        return "N/A"
    # gabung semua digit 2-6 angka
    digits = re.findall(r'\d{2,6}', message)
...
# ===== CHECK SMS DARI MULTI API =====
async def check_sms():
    global sent_messages
    async with aiohttp.ClientSession() as session:
        for api in API_LIST:
            for attempt in range(2 if api["name"]=="TIMESMS" else 1):
                try:
                    async with session.get(api["url"], params={"token": api["token"], "records": ""}, timeout=15) as resp:
                        try:
                            data = await resp.json()
                        except:
                            if api["name"]=="TIMESMS" and attempt==0:
                                continue
                    ...
                    # untuk TIMESMS API yang bentuk {status, total, data}
                    if api["name"]=="TIMESMS":
                        data = data.get("data", [])

                    for item in data:
                        # SESUAI FORMAT: [app_name, phone, message, dt]
                        if api["name"]=="TIMESMS":
                            app_name = item.get("cli","Unknown")
                            phone = item.get("num","Unknown")
                            message = item.get("message","")
                            time = item.get("dt","Unknown")
                        else:
                            if not isinstance(item, list) or len(item)<4:
                                continue
                            app_name, phone, message, time = item[0], item[1], item[2], item[3]
                    ...
                    if attempt==1 or api["name"]!="TIMESMS":
